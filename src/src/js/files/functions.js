...
// Вертикальные функции (для экранов > 500px)
// const _slideUpVertical = (target, duration = 300, showmore = 0) => {
...
// 	}
// };
// const _slideDownVertical = (target, duration = 300, showmore = 0) => {
// 	if (!target.classList.contains('_slide')) {
// 		target.classList.add('_slide');
...
// 	}
// };
const _slideUpVertical = (target, duration = 500, showmore = 0) => {
	if (!target.classList.contains('_slide')) {
		target.classList.add('_slide');
...
const _slideDownVertical = (target, duration = 500, showmore = 0) => {
	if (!target.classList.contains('_slide')) {
		target.classList.add('_slide');
		target.hidden = target.hidden ? false : null;
		showmore ? target.style.removeProperty('height') : null;
...
const _slideUpHorizontal = (target, duration = 500, showmore = 0) => {
	...
	window.setTimeout(() => {
		target.hidden = !showmore ? true : false;
		if (!showmore) target.style.removeProperty('width');
		...
		document.dispatchEvent(new CustomEvent('slideLeftDone', { detail: { target } }));
	}, duration);
};
const _slideDownHorizontal = (target, duration = 500, showmore = 0) => {
	if (target.classList.contains('_slide')) return;
	target.classList.add('_slide');
	...
	window.setTimeout(() => {
		...
		document.dispatchEvent(new CustomEvent('slideRightDone', { detail: { target } }));
	}, duration);
};

// Динамическое переключение на основе медиа-запроса
const mediaQuery = window.matchMedia('(max-width: 767.98px)');

// Экспортируемые функции, которые автоматически выбирают реализацию
export const _slideUp = (target, duration = 500, showmore = 0) => {
	if (mediaQuery.matches) {
		return _slideUpVertical(target, duration, showmore);
	} else {
		return _slideUpHorizontal(target, duration, showmore);
	}
};

export const _slideDown = (target, duration = 500, showmore = 0) => {
	if (mediaQuery.matches) {
		return _slideDownVertical(target, duration, showmore);
	} else {
		return _slideDownHorizontal(target, duration, showmore);
	}
};
...
// 			let spollerItems = spollersBlock.querySelectorAll('.spollers-menu__item');
// 			if (spollerItems.length) {
// 				//spollerItems = Array.from(spollerItems).filter(item => item.closest('[data-spollers]') === spollersBlock);
// 				spollerItems.forEach(spollerItem => {
// 					let spollerTitle = spollerItem.querySelector('.spollers-menu__title');
// 					if (hideSpollerBody) {
// 						spollerTitle.removeAttribute('tabindex');
// 						if (!spollerItem.hasAttribute('data-open')) {
// 							//spollerItem.open = false;
// 							spollerItem.setAttribute('data-spoller-open', 'false');
// 							spollerTitle.nextElementSibling.hidden = true;
// 						} else {
// 							spollerTitle.classList.add('_spoller-active');
// 							spollerItem.setAttribute('data-spoller-open', 'true');
...
// 			const el = e.target;
// 			if (el.closest('.spollers-menu__title') && el.closest('[data-spollers]')) {
// 				e.preventDefault();
// 				if (el.closest('[data-spollers]').classList.contains('_spoller-init')) {
// 					const spollerTitle = el.closest('.spollers-menu__title');
// 					const spollerBlock = spollerTitle.closest('.spollers-menu__item');
// 					const spollersBlock = spollerTitle.closest('[data-spollers]');
// 					const oneSpoller = spollersBlock.hasAttribute('data-one-spoller');
// 					const scrollSpoller = spollerBlock.hasAttribute('data-spoller-scroll');
// 					const spollerSpeed = spollersBlock.dataset.spollersSpeed ? parseInt(spollersBlock.dataset.spollersSpeed) : 500;
// 					if (!spollersBlock.querySelectorAll('._slide').length) {
// 						if (oneSpoller && !spollerBlock.getAttribute('data-spoller-open')) { // !spollerBlock.open
// 							hideSpollersBody(spollersBlock);
// 						}

// 						//!spollerBlock.open ? spollerBlock.open = true : setTimeout(() => { spollerBlock.open = false }, spollerSpeed);
// 						!spollerBlock.getAttribute('data-spoller-open') ? spollerBlock.setAttribute('data-spoller-open', 'true') : setTimeout(() => { spollerBlock.setAttribute('data-spoller-open', 'false') }, spollerSpeed);
...
// 							setTimeout(() => { spollerCloseBlock.setAttribute('data-spoller-open', 'false') }, spollerSpeed);
// 						}
// 					});
// 				}
// 			}
...
// 			const spollerActiveBlock = spollersBlock.querySelector('.spollers-menu__item[open]');
// 			if (spollerActiveBlock && !spollersBlock.querySelectorAll('._slide').length) {
// 				const spollerActiveTitle = spollerActiveBlock.querySelector('.spollers-menu__title');
// 				const spollerSpeed = spollersBlock.dataset.spollersSpeed ? parseInt(spollersBlock.dataset.spollersSpeed) : 500;
// 				spollerActiveTitle.classList.remove('_spoller-active');
// 				_slideUp(spollerActiveTitle.nextElementSibling, spollerSpeed);
// 				//setTimeout(() => { spollerActiveBlock.open = false }, spollerSpeed);
// 				setTimeout(() => { spollerActiveBlock.setAttribute('data-spoller-open', 'false') }, spollerSpeed);
// 			}
// 		}
// 	}
// }
		...
		function setSpollerAction(e) {
			...
			if (el.closest('[data-spoller]')) {
				...
				if (!spollersBlock.querySelectorAll('._slide').length) {
					...
					spollerTitle.classList.toggle('_spoller-active');
					console.log(spollerTitle.parentElement)
...
export function menuInit() {
	...
	if (document.querySelector(".icon-contacts-menu")) {
		document.addEventListener("click", function (e) {
			if (bodyLockStatus && e.target.closest('.icon-contacts-menu')) {
				bodyLockToggle();
				document.documentElement.classList.toggle("contacts-menu-open");
			}
		});
	};
}
